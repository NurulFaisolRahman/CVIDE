    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Tooltip untuk tombol yang tidak bisa diedit */
    .link-act[style*="cursor:not-allowed"]:hover {
        background: transparent;
    }

    /* Versi badge di tabel */
    .version-badge-table {
        display: inline-block;
        background: #e2e8f0;
        color: #475569;
...
.google-link-info {
    ...
    <ul class="nav nav-tabs mb-4" style="border-bottom: 1px solid var(--border);">
        <li class="nav-item">
            <button class="nav-link active" id="tab-api-btn" style="background: transparent; border: none; padding: 10px 20px; font-weight: 600; color: var(--blue-600); border-bottom: 2px solid var(--blue-600);" onclick="switchDataTab('api')">
                <i class="fas fa-cloud-download-alt"></i> Data dari API
            </button>
        </li>
        <li class="nav-item">
            <button class="nav-link" id="tab-manual-btn" style="background: transparent; border: none; padding: 10px 20px; font-weight: 600; color: var(--text-2);" onclick="switchDataTab('manual')">
    ...
    <!-- ==================== DATA MANUAL PANEL ==================== -->
    <div id="manual-data-panel" style="display: none;">
        <!-- Form Input Manual -->
        <div class="card mb-4">
            <div class="card-head">
                <div class="card-title">
                    <i class="fas fa-plus-circle"></i> Input Data Indikator Manual
                </div>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select id="manual-kode-api" class="form-control" onchange="loadManualDataByKode()">
                                <option value="">-- Pilih Kode API --</option>
                            </select>
                            <small class="text-muted">Pilih konfigurasi API yang sudah ada</small>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            <label class="form-label">Satuan</label>
                            <input type="text" id="manual-satuan" class="form-control" placeholder="Contoh: Rupiah, Persen, Unit">
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" id="manual-tahun" class="form-control" placeholder="2024">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            <label class="form-label">Nilai</label>
                            <input type="text" id="manual-nilai" class="form-control" placeholder="Contoh: 1000000">
                        </div>
                    </div>
                </div>
                
                <div class="form-group mb-3">
                    <button class="btn btn-primary" onclick="addManualDataRow()">
                ...
                <div class="table-responsive mt-3">
                    <table class="tbl" id="manual-data-table">
                        <thead>
                            <tr>
                                <th>Tahun</th>
                                <th>Nilai</th>
                        ...
                        <tbody id="manual-data-tbody">
                            <tr id="manual-empty-row">
                                <td colspan="3" class="text-center py-4" style="color: var(--text-3);">
                                    <i class="fas fa-database"></i> Belum ada data. Klik "Tambah Data"
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="form-actions mt-3">
                    ...
                    <button class="btn btn-secondary" onclick="resetManualForm()">
        ...
        <!-- Daftar Data Manual yang Tersimpan -->
        <div class="card">
            <div class="card-head">
                <div class="card-title">
                    <i class="fas fa-list"></i> Data Manual Tersimpan
                </div>
                <span class="card-action" onclick="loadSavedManualData()">Refresh</span>
            </div>
            <div class="table-responsive">
                <table class="tbl">
                    <thead>
    ...
    <div id="modalApiConfig" class="modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:1002; align-items:center; justify-content:center;">
        <div style="background:#fff; border-radius:20px; width:700px; max-width:90%; max-height:85vh; overflow:auto; padding:24px;">
            ...
            <!-- ✅ TAMBAHKAN FORM DI SINI (pembuka) -->
            <form id="formApiConfig">
                ...
                <div class="form-group" style="margin-bottom:15px;">
                    ...
                    <small class="text-muted">URL lengkap API yang akan dipanggil</small>
                </div>
   ...
   <!-- Modal Upload Dokumen (DENGAN LINK GOOGLE DOCS) -->
<div id="modalUpload" class="modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:1000; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:16px; width:550px; max-width:90%; padding:24px;">
        <div style="display:flex; justify-content:space-between; margin-bottom:20px;">
            <h3 style="font-size:18px;">Tambah Dokumen</h3>
        ...
        <form id="formUploadDokumen" enctype="multipart/form-data">
            ...
            <!-- Nama Dokumen -->
            <div class="form-group" style="margin-bottom:15px;">
                <label class="form-label">Nama Dokumen <span style="color:red;">*</span></label>
                <input type="text" name="nama_dokumen" class="form-control" required placeholder="Contoh: Rencana Kerja 2025">
            </div>
            
            <!-- OPSI UPLOAD: Pilih metode -->
            <div class="form-group" style="margin-bottom:15px;">
                <label class="form-label">Metode Input <span style="color:red;">*</span></label>
                <select id="upload_method" class="form-control" onchange="toggleUploadMethod()">
                    <option value="file">📁 Upload File (.docx)</option>
                    <option value="link">🔗 Link Google Docs</option>
                </select>
            </div>
            ...
            <div id="upload-file-section">
                <div class="form-group" style="margin-bottom:15px;">
                    <label class="form-label">File Dokumen <span style="color:red;">*</span></label>
                    <input type="file" name="file_dokumen" class="form-control" accept=".docx">
                    <small class="text-muted">Hanya file .docx yang didukung untuk edit online</small>
                </div>
            </div>
            
            <!-- Link Google Docs (hidden by default) -->
            <div id="upload-link-section" style="display:none;">
                <div class="form-group" style="margin-bottom:15px;">
                    <label class="form-label">Link Google Docs <span style="color:red;">*</span></label>
                    <input type="url" name="google_docs_link" id="google_docs_link" class="form-control" 
                           placeholder="https://docs.google.com/document/d/xxxxx/edit">
                    <small class="text-muted">
                        <i class="fas fa-info-circle"></i> 
                        Pastikan link sudah di-share dengan akses "Siapa saja dengan link dapat mengedit"
                    </small>
                </div>
                
                <div class="form-group" style="margin-bottom:15px;">
                    <label class="form-label">Preview (Opsional)</label>
                    <div id="link-preview" style="background:#f0f4ff; padding:12px; border-radius:8px; font-size:12px; display:none;">
                        <i class="fas fa-check-circle" style="color:green;"></i> 
                        <span id="link-preview-text"></span>
                    </div>
                </div>
            </div>
    ...
    const method = $('#upload_method').val();
    const fileSection = $('#upload-file-section');
    const linkSection = $('#upload-link-section');
    const fileInput = $('input[name="file_dokumen"]');
    const linkInput = $('#google_docs_link');
...
// Preview link Google Docs
$('#google_docs_link').on('input', function() {
    const link = $(this).val();
    if (link && link.includes('docs.google.com/document/d/')) {
        const match = link.match(/\/d\/([a-zA-Z0-9_-]+)/);
        if (match) {
            const docId = match[1];
            $('#link-preview-text').html(`Link valid! Document ID: ${docId.substring(0, 10)}...`);
            $('#link-preview').show();
        } else {
            $('#link-preview-text').html('Format link tidak valid. Pastikan link Google Docs yang benar.');
            $('#link-preview').show().css('background', '#fee2e2');
        }
    } else {
        $('#link-preview').hide();
    }
}).on('focus', function() {
    if ($(this).val() && !$('#link-preview').is(':visible')) {
        $('#link-preview').show();
    }
});
    ...
    // RESET ke mode file setiap modal dibuka
    $('#upload_method').val('file');
    toggleUploadMethod();
    ...
    // Base URL untuk edit dokumen
    const EDIT_URL = '<?= base_url("IDE/edit_dokumen/") ?>';
    
    tbody.innerHTML = dokumen.map((doc, index) => {
        // ========== CEK APAKAH DOKUMEN DARI GOOGLE DOCS ==========
        const isGoogleDocs = doc.google_docs_link && doc.google_docs_link.trim() !== '';
        
        // Tentukan thumbnail berdasarkan tipe file atau Google Docs
        let thumbnailHtml = '';
        const baseUrl = '<?= base_url() ?>';
        
        if (isGoogleDocs) {
            // Thumbnail khusus untuk Google Docs
            thumbnailHtml = `<div onclick="previewDokumen('${doc.file_path}', '${doc.file_type}', '${escapeHtml(doc.nama_dokumen)}', ${doc.id_dokumen})" 
                style="width:50px; height:50px; background:#e8f0fe; border-radius:8px; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:24px;">
                <i class="fab fa-google" style="color:#1a73e8; font-size:24px;"></i>
            </div>`;
        } else if (doc.file_type && doc.file_type.startsWith('image/')) {
            const thumbUrl = doc.thumbnail ? baseUrl + doc.thumbnail : baseUrl + doc.file_path;
            thumbnailHtml = `<img src="${thumbUrl}" class="doc-thumbnail" onclick="previewDokumen('${doc.file_path}', '${doc.file_type}', '${escapeHtml(doc.nama_dokumen)}', ${doc.id_dokumen})" 
        ...
        // ========== DETEKSI TIPE FILE UNTUK FILE UPLOAD ==========
        const isDocx = (
            doc.file_type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ||
            doc.file_type === 'application/vnd.openxmlformats-officedocument.word' ||
            (doc.file_type && doc.file_type.includes('wordprocessingml')) ||
            (doc.file_type && doc.file_type.includes('openxmlformats')) ||
            (doc.file_name && doc.file_name.toLowerCase().endsWith('.docx'))
        );
        const isPdf = doc.file_type === 'application/pdf';
        
        // ========== TOMBOL EDIT BERDASARKAN JENIS DOKUMEN ==========
        let editButtonHtml = '';
        
        if (isGoogleDocs) {
            // Dokumen dengan link Google Docs
            editButtonHtml = `<a href="${EDIT_URL}${doc.id_dokumen}" class="link-act" style="margin-right:12px; color:#1a73e8;" target="_blank" title="Edit via Google Docs">
                <i class="fab fa-google"></i> Edit Google Docs
            </a>`;
        } else if (isDocx) {
            // File DOCX biasa
            editButtonHtml = `<a href="${EDIT_URL}${doc.id_dokumen}" class="link-act" style="margin-right:12px; color:#f59e0b;" title="Edit dokumen ini">
                <i class="fas fa-edit"></i> Edit
            </a>`;
        } else if (isPdf) {
            editButtonHtml = `<span class="link-act" style="margin-right:12px; color:#9ca3af; cursor:not-allowed;" title="File PDF tidak dapat diedit langsung. Silakan upload ulang file yang sudah diedit.">
                <i class="fas fa-lock"></i> Tidak Bisa Edit
            </span>`;
        }
        
        // ========== BADGE UNTUK GOOGLE DOCS ==========
        let googleBadge = '';
        if (isGoogleDocs) {
            googleBadge = `<span class="google-docs-badge" style="display:inline-flex; align-items:center; gap:4px; background:#e8f0fe; color:#1a73e8; font-size:9px; font-weight:500; padding:2px 8px; border-radius:12px; margin-left:8px;">
                <i class="fab fa-google" style="font-size:10px;"></i> Google Docs
            </span>`;
        }
        
        return `
        <tr style="border-bottom:1px solid var(--border);" data-id="${doc.id_dokumen}" data-kategori="${kategori}" data-file-type="${doc.file_type}">
            ...
            <td style="padding:12px 8px;">
                ...
                ${googleBadge}
                <br><span style="font-size:10px; color:var(--text-3);">${isGoogleDocs ? 'Google Docs Online' : fileSize}</span>
            </td>
            <td style="padding:12px 8px;">
            ...
            <td style="padding:12px 8px;">
                <i class="fas fa-user-circle" style="color:#6b7280; font-size:12px;"></i>
            ...
            <td style="padding:12px 8px; white-space:nowrap;">
                <span class="link-act" onclick="previewDokumen('${doc.file_path}', '${doc.file_type}', '${escapeHtml(doc.nama_dokumen)}', ${doc.id_dokumen})" style="margin-right:12px; cursor:pointer; color:#2563eb;">
                    <i class="fas fa-eye"></i> Lihat
                </span>
                ${editButtonHtml}
                <span class="link-danger" onclick="deleteDokumen(${doc.id_dokumen})" style="cursor:pointer; color:#dc2626;">
    ...
    // ========== DEBUG: RINGKASAN SETELAH RENDER ==========
    // Gunakan kondisi fleksibel yang sama untuk ringkasan
    const docxCount = dokumen.filter(doc => 
        doc.file_type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ||
        doc.file_type === 'application/vnd.openxmlformats-officedocument.word' ||
        (doc.file_type && doc.file_type.includes('wordprocessingml')) ||
        (doc.file_type && doc.file_type.includes('openxmlformats')) ||
    ...
    const pdfCount = dokumen.filter(doc => doc.file_type === 'application/pdf').length;
    const otherCount = dokumen.length - docxCount - pdfCount;
    
    console.log('========== RINGKASAN ==========');
    console.log(`Total dokumen: ${dokumen.length}`);
    console.log(`File DOCX: ${docxCount} (tombol Edit akan muncul)`);
    console.log(`File PDF: ${pdfCount} (tidak bisa edit langsung)`);
    console.log(`File lainnya: ${otherCount}`);
    console.log('================================');
}

    // ================= FUNGSI PREVIEW DOKUMEN YANG DIPERBARUI =================
    function previewDokumen(filePath, fileType, fileName, idDokumen = null) {
        // SIMPAN ke hidden input untuk digunakan saat print
        if (idDokumen) {
            const idInput = document.getElementById('preview-dokumen-id');
            const namaInput = document.getElementById('preview-dokumen-nama');
            const pathInput = document.getElementById('preview-dokumen-path');
            
            if (idInput) idInput.value = idDokumen;
            if (namaInput) namaInput.value = fileName;
            if (pathInput) pathInput.value = filePath;
        }
        
        // Tampilkan preview
        const previewTitle = document.getElementById('previewTitle');
        if (previewTitle) previewTitle.textContent = fileName || 'Preview Dokumen';
        ...
        // Tampilkan konten preview berdasarkan tipe file
        if (fileType && fileType.startsWith('image/')) {
            ...
            // Untuk file DOCX, tampilkan link download dan edit
            const editUrl = '<?= base_url("IDE/edit_dokumen/") ?>' + idDokumen;
            previewDiv.innerHTML = `
                <div style="padding:60px 20px; text-align:center;">
                    <div style="font-size:64px; margin-bottom:20px;">📝</div>
                    <p style="margin-bottom:20px; color:var(--text-2);">Dokumen Word (.docx) dapat diedit</p>
                    <div style="display:flex; gap:15px; justify-content:center;">
                        <a href="${fullUrl}" class="btn btn-primary" download target="_blank">
                            <i class="fas fa-download"></i> Download
                        </a>
                        <a href="${editUrl}" class="btn btn-warning" style="background:#f59e0b; color:white;">
    ...
    // ========== SUBMIT HANDLER UNTUK UPLOAD DOKUMEN ==========
$('#formUploadDokumen').off('submit').on('submit', function(e) {
    ...
    if (method === 'file') {
        ...
        if (fileExt !== 'docx') {
            Swal.fire('Error!', 'Hanya file .docx yang didukung untuk edit online', 'error');
            return;
        }
    } else {
        // Mode Link Google Docs
        const link = $('#google_docs_link').val();
        if (!link || link.trim() === '') {
            Swal.fire('Error!', 'Masukkan link Google Docs terlebih dahulu', 'error');
            return;
        }
        if (!link.includes('docs.google.com/document/d/')) {
            Swal.fire('Error!', 'Link tidak valid. Pastikan link Google Docs yang benar', 'error');
            return;
        }
    }
    
    // Validasi nama dokumen
    const namaDokumen = $('input[name="nama_dokumen"]').val().trim();
    if (!namaDokumen) {
        Swal.fire('Error!', 'Nama dokumen harus diisi', 'error');
        return;
    }
    ...
    if (method === 'file') {
        formData.delete('google_docs_link');
    } else {
        formData.delete('file_dokumen');
    }
    
    // Tambahkan method ke FormData
    ...
    Swal.fire({
        title: 'Menyimpan...',
        text: 'Mohon tunggu, sedang memproses dokumen',
        allowOutsideClick: false,
        didOpen: () => Swal.showLoading()
    });
                ...
                <div class="col-md-4 col-lg-3">
                    <div class="card data-card" data-kode="${config.kode_api}" style="cursor:pointer; transition:all 0.2s; height:100%;">
                        <div class="card-body">
                            ...
                            <p class="card-text text-center text-muted small">Kode: ${escapeHtml(config.kode_api)}</p>
    ...
    // ==================== FUNGSI SELECT DATA (HANYA VIA BACKEND) ====================
    async function selectDataByKode(kode) {
        ...
        if (!container) {
            console.error('Element selected-data-container tidak ditemukan');
            return;
        }
        ...
        // Tampilkan loading
        $('#selected-indikator').text('Memuat dari server...');
        $('#selected-produsen').text('-');
        $('#selected-urusan').text('-');
        $('#selected-satuan').text('-');
        $('#selected-data-tbody').html(`
            <tr>
                <td colspan="3" class="text-center py-5">
                    <div class="modern-spinner"></div>
        ...
        // HANYA PANGGIL API VIA BACKEND (jangan pakai fetch langsung)
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                $('#selected-data-tbody').html(`
                    <tr>
                        <td colspan="3" class="text-center py-5" style="color:red;">
                            <p>❌ Error: ${error}</p>
                            <p>Silakan periksa koneksi internet</p>
    ...
    function openApiModal(id = null) {
        if (id) {
            ...
            if (config) {
                ...
                $('#url_api').val(config.url_api);  // ← Ini yang benar
    ...
    // ==================== VARIABEL GLOBAL ====================
    let manualDataRows = [];
    let currentEditingManualId = null;

    // ==================== SWITCH TAB ====================
    function switchDataTab(tab) {
        const apiPanel = document.querySelector('#data-cards-container');
        const manualPanel = document.getElementById('manual-data-panel');
        ...
        if (tab === 'api') {
            ...
            tabApiBtn.style.color = 'var(--blue-600)';
            tabApiBtn.style.borderBottom = '2px solid var(--blue-600)';
            tabManualBtn.classList.remove('active');
            tabManualBtn.style.color = 'var(--text-2)';
            tabManualBtn.style.borderBottom = 'none';
            loadApiConfigs();
        } else {
            apiPanel.style.display = 'none';
            manualPanel.style.display = 'block';
            tabManualBtn.classList.add('active');
            tabManualBtn.style.color = 'var(--blue-600)';
            tabManualBtn.style.borderBottom = '2px solid var(--blue-600)';
            tabApiBtn.classList.remove('active');
            tabApiBtn.style.color = 'var(--text-2)';
            tabApiBtn.style.borderBottom = 'none';
            loadManualSelectOptions();
            loadSavedManualData();
        }
    }

    // ==================== LOAD KODE API UNTUK DROPDOWN ====================
    function loadManualSelectOptions() {
        $.ajax({
            url: baseUrlData + 'IDE/get_api_configs',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success') {
                    let options = '<option value="">-- Pilih Kode API --</option>';
                    response.data.forEach(function(config) {
                        options += `<option value="${escapeHtml(config.kode_api)}" data-nama="${escapeHtml(config.nama_data)}">${escapeHtml(config.kode_api)} - ${escapeHtml(config.nama_data)}</option>`;
                    });
                    $('#manual-kode-api').html(options);
                }
            }
        });
    }

    // ==================== LOAD MANUAL DATA BY KODE ====================
    function loadManualDataByKode() {
        const kode = $('#manual-kode-api').val();
        if (!kode) {
            resetManualForm();
            return;
        }
        ...
        $.ajax({
            url: baseUrlData + 'IDE/get_manual_data_by_kode',
            type: 'GET',
            data: { kode: kode },
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success' && response.data) {
                    currentEditingManualId = response.data.id;
                    $('#manual-satuan').val(response.data.satuan || '');
                    manualDataRows = response.data.series || [];
                    renderManualDataTable();
                } else {
                    currentEditingManualId = null;
                    manualDataRows = [];
                    renderManualDataTable();
                }
            },
    ...
    // ==================== RENDER MANUAL DATA TABLE ====================
    function renderManualDataTable() {
        const tbody = $('#manual-data-tbody');
        
        if (!manualDataRows || manualDataRows.length === 0) {
            tbody.html(`
                <tr id="manual-empty-row">
                    <td colspan="3" class="text-center py-4" style="color: var(--text-3);">
                        <i class="fas fa-database"></i> Belum ada data. Klik "Tambah Data"
        ...
        let html = '';
        manualDataRows.forEach((row, index) => {
            html += `
                <tr>
                    <td>${escapeHtml(row.tahun)}</td>
                    <td>${formatNumberRibuan(row.nilai)}</td>
                    <td>
                        <button class="btn btn-sm btn-outline-danger" onclick="removeManualDataRow(${index})">
                            <i class="fas fa-trash"></i>
                        </button>
                    </td>
                </tr>
    ...
    function addManualDataRow() {
        const tahun = $('#manual-tahun').val().trim();
        const nilai = $('#manual-nilai').val().trim();
        ...
        if (!nilai) {
            Swal.fire('Error', 'Nilai harus diisi', 'error');
            return;
        }
        
        // Cek duplikat tahun
        const exists = manualDataRows.some(row => row.tahun == tahun);
        if (exists) {
            Swal.fire('Error', `Data untuk tahun ${tahun} sudah ada`, 'error');
            return;
        }
        ...
        manualDataRows.sort((a, b) => a.tahun - b.tahun);
    ...
    // ==================== REMOVE MANUAL DATA ROW ====================
    function removeManualDataRow(index) {
        manualDataRows.splice(index, 1);
        renderManualDataTable();
    }
    ...
    function saveManualData() {
        const kode = $('#manual-kode-api').val();
        const satuan = $('#manual-satuan').val().trim();
        
        if (!kode) {
            Swal.fire('Error', 'Pilih Kode API terlebih dahulu', 'error');
            return;
        }
        
        if (manualDataRows.length === 0) {
            Swal.fire('Error', 'Minimal 1 data tahun harus diisi', 'error');
            return;
        }
        
        // Ambil nama data dari dropdown
        const selectedOption = $('#manual-kode-api option:selected');
        const namaData = selectedOption.data('nama') || kode;
        
        // Validasi data series
        let isValid = true;
        manualDataRows.forEach(row => {
            if (!row.tahun || isNaN(row.nilai)) {
                isValid = false;
            }
        });
        
        if (!isValid) {
            Swal.fire('Error', 'Data tahun dan nilai tidak valid', 'error');
            return;
        }
        ...
        const postData = {
            ...
            nama_data: namaData,
            satuan: satuan,
            data_series: JSON.stringify(manualDataRows),
            id_manual: currentEditingManualId || ''
        };
        
        console.log('Sending data:', postData);
        
        $.ajax({
            url: baseUrlData + 'IDE/save_manual_data',
            type: 'POST',
            data: postData,
            dataType: 'json',
            timeout: 30000,
    ...
    function loadSavedManualData() {
        $('#saved-manual-tbody').html(`
            <tr><td colspan="6" style="text-align:center;padding:40px;">
                <div class="modern-spinner"></div>
                <p>Memuat data...</p>
            </div></td>
        ...
        $.ajax({
            url: baseUrlData + 'IDE/get_all_manual_data',
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.status === 'success' && response.data.length > 0) {
                    ...
                    response.data.forEach(function(item) {
                        const series = JSON.parse(item.data_series);
                        html += `
                            <tr>
                                <td><code>${escapeHtml(item.kode_api)}</code></td>
                                <td><strong>${escapeHtml(item.indikator)}</strong></td>
                                <td>${escapeHtml(item.satuan || '-')}</td>
                                <td>${series.length} tahun</td>
                                <td>${item.updated_at_formatted || item.created_at_formatted}</td>
                                <td>
                                    <button class="btn btn-sm btn-outline-primary" onclick="editManualData('${escapeHtml(item.kode_api)}')">
                                        <i class="fas fa-edit"></i> Edit
                                    </button>
                                    <button class="btn btn-sm btn-outline-danger ms-1" onclick="deleteManualData('${escapeHtml(item.kode_api)}')">
                                        <i class="fas fa-trash"></i> Hapus
                                    </button>
                                </td>
                            </tr>
                    ...
                    $('#saved-manual-tbody').html(html);
                } else {
                    $('#saved-manual-tbody').html(`
                        <tr><td colspan="6" class="empty-state-table">
                            <div class="icon">📭</div>
                            <p>Belum ada data manual yang tersimpan</p>
                            <p style="font-size:12px;">Gunakan form di atas untuk menambahkan data</p>
                        </div></td>
                    `);
                }
            },
    ...
    // ==================== EDIT MANUAL DATA ====================
    function editManualData(kode) {
        $('#manual-kode-api').val(kode);
        $('#manual-kode-api').trigger('change');
        // Scroll ke form
        document.getElementById('manual-data-panel').scrollIntoView({ behavior: 'smooth' });
    }

    // ==================== DELETE MANUAL DATA ====================
    function deleteManualData(kode) {
        Swal.fire({
            title: 'Hapus Data?',
            text: `Anda yakin ingin menghapus data manual untuk kode "${kode}"?`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc2626',
            confirmButtonText: 'Ya, Hapus!',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: baseUrlData + 'IDE/delete_manual_data',
                    type: 'POST',
                    data: { kode_api: kode },
                    dataType: 'json',
                    success: function(response) {
    ...
    // ==================== MODIFIED selectDataByKode (Prioritaskan Manual Data) ====================
    async function selectDataByKode(kode) {
        console.log('Selecting data by kode:', kode);
        currentSelectedKode = kode;
        ...
        $('#selected-indikator').text('Memuat data...');
        $('#selected-produsen').text('-');
        $('#selected-urusan').text('-');
        $('#selected-satuan').text('-');
        $('#selected-data-tbody').html(`
        ...
        // Cek apakah ada data manual terlebih dahulu
        $.ajax({
            url: baseUrlData + 'IDE/get_manual_data_by_kode',
            type: 'GET',
            data: { kode: kode },
            dataType: 'json',
            timeout: 5000,
            success: function(manualResponse) {
                if (manualResponse.status === 'success' && manualResponse.data && manualResponse.data.series.length > 0) {
                    // Gunakan data manual
                    const manualData = manualResponse.data;
                    currentDataIndikator = {
                        indikator: manualData.indikator,
                        produsen: 'Input Manual',
                        urusan: 'Data Manual',
                        satuan: manualData.satuan || '-',
                        series: manualData.series
                    };
                    currentDataSeries = manualData.series;
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Data Manual Dimuat',
                        text: `${currentDataSeries.length} tahun data dari input manual`,
                        timer: 2000,
                        showConfirmButton: false
                    });
                    ...
                    // Tidak ada data manual, coba dari API
                    loadDataFromApi(kode, container);
                }
            },
            error: function() {
                // Jika error cek manual, coba dari API
                loadDataFromApi(kode, container);
            }
        });
    }

    function loadDataFromApi(kode, container) {
        $.ajax({
            url: baseUrlData + 'IDE/load_data_from_api',
            type: 'GET',
            data: { kode: kode },
            ...
            success: function(response) {
                if (response.status === 'success' && response.data && response.data.series && response.data.series.length > 0) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Data API Berhasil Dimuat',
                        text: `${currentDataSeries.length} tahun data`,
                        timer: 2000,
                        showConfirmButton: false
                    });
                    ...
                    $('#selected-data-tbody').html(`
                        <tr><td colspan="3" class="text-center py-5" style="color:#dc2626;">
                            <div style="font-size:48px; margin-bottom:15px;">⚠️</div>
                            <p>❌ ${response.message || 'Data tidak ditemukan'}</p>
                            <div class="mt-3">
                                <p class="text-muted">💡 Tips: Gunakan menu "Input Manual" untuk menambahkan data secara manual</p>
                                <button class="btn btn-primary btn-sm mt-2" onclick="switchDataTab('manual')">
                                    <i class="fas fa-edit"></i> Input Manual
                                </button>
                            </div>
                        </div></tr>
            ...
            error: function() {
                $('#selected-data-tbody').html(`
                    <tr><td colspan="3" class="text-center py-5" style="color:#dc2626;">
                        <p>❌ Gagal mengambil data dari API</p>
                        <div class="mt-3">
                            <p class="text-muted">💡 Gunakan menu "Input Manual" untuk menambahkan data secara manual</p>
                            <button class="btn btn-primary btn-sm mt-2" onclick="switchDataTab('manual')">
                                <i class="fas fa-edit"></i> Input Manual
                            </button>
                        </div>
