...
<div class="app-container">
    ...
    <div class="admin-login-section" id="adminLoginSection">
        <h3 style="color: white; margin: 0; text-align: center; font-size: 16px;">
            <i class="fas fa-lock"></i> Akses Admin
        </h3>
        <input type="email" class="admin-input" id="adminEmail" placeholder="Masukkan email admin...">
        <input type="password" class="admin-input" id="adminPassword" placeholder="Masukkan password admin...">
    ...
    <!-- Admin Access Notice -->
    <div class="admin-access-notice" id="adminAccessNotice">
        <i class="fas fa-user-shield"></i> <strong>Akses Admin Aktif</strong>
        <p style="font-size: 12px; margin: 5px 0;">Anda login sebagai admin. Link dapat diakses.</p>
        <button onclick="logoutAdmin()" style="background: none; border: 1px solid white; color: white; padding: 5px 10px; border-radius: 6px; font-size: 11px; cursor: pointer;">
    ...
    <div class="locked-message" id="lockedMessage">
        ...
        <h3>Waktu Pengisian Telah Berakhir</h3>
        <p>Masa pengisian LKPJ Bupati Situbondo Tahun 2025 telah berakhir pada tanggal 13 Januari 2026 pukul 23:59:59.</p>
        <p>Jika Anda adalah admin yang berwenang, silakan login untuk mengakses tautan.</p>
        <button onclick="showAdminLogin()" class="admin-btn" style="margin-top: 15px; background: linear-gradient(135deg, #6c757d, #495057);">
...
<script>
    // Daftar email admin yang diizinkan (bisa ditambah sesuai kebutuhan)
    const ADMIN_EMAILS = [
        'lkpjsitubondo2025@gmail.com',
    ];
    
    // Password admin (dalam real implementation, ini seharusnya di server)
    const ADMIN_PASSWORD = 'situbondo.25';
    
    // Deadline waktu pengisian
    const DEADLINE_DATE = new Date("Jan 13, 2026 23:59:59").getTime();
    
    // Status login admin
    let isAdminLoggedIn = false;
    
    // Check deadline and admin status on load
    document.addEventListener('DOMContentLoaded', function() {
        checkDeadline();
        
        // Check if admin is already logged in from localStorage
        const savedAdmin = localStorage.getItem('lkpj_admin');
        if (savedAdmin) {
            try {
                const adminData = JSON.parse(savedAdmin);
                if (adminData.email && adminData.timestamp) {
                    // Check if login is still valid (24 hours)
                    const now = new Date().getTime();
                    const loginTime = new Date(adminData.timestamp).getTime();
                    const hoursDiff = (now - loginTime) / (1000 * 60 * 60);
                    
                    if (hoursDiff < 24 && ADMIN_EMAILS.includes(adminData.email)) {
                        isAdminLoggedIn = true;
                        showAdminContent();
                    } else {
                        localStorage.removeItem('lkpj_admin');
                    }
                }
            } catch (e) {
                localStorage.removeItem('lkpj_admin');
    ...
    function checkDeadline() {
        ...
        // If deadline has passed and user is not admin, show locked message
        if (distance < 0 && !isAdminLoggedIn) {
            showLockedMessage();
        } else if (isAdminLoggedIn) {
            // Admin is logged in, show content
            showAdminContent();
        } else {
            // Deadline not passed yet, show content
            showRegularContent();
        }
    }
    ...
    function updateCountdown() {
        ...
        // If deadline passed, show expired message
        if (distance < 0) {
            clearInterval(countdownInterval);
            document.getElementById("countdown").innerHTML = "<span style='font-weight:800; letter-spacing:1px; color:#ff6b6b;'>WAKTU PELAPORAN BERAKHIR</span>";
        }
    }
    ...
    function showLockedMessage() {
        ...
        document.getElementById('adminAccessNotice').classList.remove('show');
    }
    
    function showAdminContent() {
        document.getElementById('lockedMessage').classList.remove('show');
        document.getElementById('linkContent').style.display = 'block';
        document.getElementById('adminLoginSection').classList.remove('show');
        document.getElementById('adminAccessNotice').classList.add('show');
    }
    ...
    function showAdminLogin() {
        document.getElementById('adminLoginSection').classList.add('show');
    }
    ...
    function verifyAdmin() {
        ...
        if (!email || !password) {
            Swal.fire({
                ...
                text: 'Silakan masukkan email dan password admin',
        ...
        if (ADMIN_EMAILS.includes(email) && password === ADMIN_PASSWORD) {
            ...
            localStorage.setItem('lkpj_admin', JSON.stringify(adminData));
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    ...
                    checkDeadline(); // This will show locked message if deadline passed
                });
            }
        });
    }
    ...
    function konfirmasiBuka(namaDinas, url) {
        if (!isAdminLoggedIn && new Date().getTime() > DEADLINE_DATE) {
            Swal.fire({
                icon: 'error',
                title: 'Akses Ditolak',
                text: 'Waktu pengisian telah berakhir. Hanya admin yang dapat mengakses.',
                confirmButtonColor: '#dc3545'
            });
            return;
        }
    ...
    // WhatsApp function remains the same
