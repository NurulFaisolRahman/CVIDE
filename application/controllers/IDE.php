...
class IDE extends CI_Controller {
  ...
  public function NTP(){
    $Data = $this->db->query("SELECT Id,NamaResponden,NIK,TanggalSurvei,KodeKualitas,Harga,_Harga FROM `ntpkonsumen`")->result_array();
    $No = 1;
		foreach ($Data as $key) {
      ...
      for ($i=0; $i < count($Harga); $i++) { 
        ...
        //     echo $No++.' '.$key['Id'].' '.$key['NIK'].' '.$key['TanggalSurvei'].' '.$Pisah[$i].' '.$Harga[$i].' '.$_Harga[$i].'<br>';
        // }
